...
def baseline_full_text(name):
    ...
    bool_list = []
    bool_non_list = []
    with open(globalparameter.raw_data_path) as f:
        reader = csv.reader(f)
        for row in reader:
            new_row = ' '.join(row)
            if name in new_row:
...
def baseline_work_exp(name):
    ...
    score = precision_score(bool_list + zero_list, bool_list_baseline + bool_non_list)
    print('baseline_precision_work_exp_of '+name+' : {}'.format(score))