...
id_positive = sorted_id_work_year['id'][:length].tolist()
id_negative = sorted_id_work_year['id'][length:1000].tolist()
id_true_positive = []
id_true_negative = []
id_false_positive = []
id_false_negative = []
...
for i in range(length):
    if cosine_similarity_other[i] in id_negative:
        id_true_negative.append(cosine_similarity_top500[i])
for i in range(length):
    if cosine_similarity_top500[i] in id_positive:
        id_true_positive.append(cosine_similarity_top500[i])
for i in range(length):  
    if cosine_similarity_top500[i] in id_negative:
        id_false_negative.append(cosine_similarity_top500[i])
for i in range(length):
    if cosine_similarity_other[i] in id_positive:
